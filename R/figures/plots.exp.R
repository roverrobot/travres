...
plot.data <- function(t, I0s, cim=1, ps, t0s, disease) {
...
if (exists("use.pdf") && use.pdf) pdf(file="deaths.exp.pdf", width=5, height=5)
print(plot.curves(rbind(ebola.data$deaths, sars.data$deaths), ylab="total deaths", m=20))
if (exists("use.pdf") && use.pdf) dev.off()

if (exists("use.pdf") && use.pdf) pdf(file="beds.exp.pdf", width=5, height=5)
